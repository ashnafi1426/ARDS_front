...
const StudentLayout = ({ children, activeOverride, onTabChange }) => {
  const { user, logout } = useAuth();
  const navigate = useNavigate();
  const location = useLocation();
  const [isSidebarOpen, setIsSidebarOpen] = useState(true);
  const activeTab = activeOverride || getActiveTab(location.pathname);

  function getActiveTab(pathname) {
    if (!pathname || pathname === '/student') return 'dashboard';
    const parts = pathname.split('/');
    const lastPart = parts[parts.length - 1];
    const validTabs = ['dashboard', 'self-check', 'notifications', 'resources', 'calendar', 'profile', 'help'];
  ...
  useEffect(() => {
    if (onTabChange) onTabChange(getActiveTab(location.pathname));
  ...
  const handleNavigation = (item) => {
    ...
    // Force focus out to prevent stuck highlight focus
  ...
  return (
    <div className="flex h-screen bg-[#0f172a] text-slate-200 overflow-hidden font-sans">
      ...
      {/* Primary Navigation Registry (Sidebar) */}
      <aside
        className={`flex flex-col flex-shrink-0 transition-all duration-500 ease-in-out bg-gradient-to-b from-[#115e59] to-[#042f2e] border-r border-white/5 shadow-3xl z-[100] ${isSidebarOpen ? 'w-[280px]' : 'w-24'}`}
      >
      ...
      <div className="flex-1 flex flex-col min-w-0 relative overflow-hidden bg-[#1e1b4b]">
        ...
        <header className="h-20 bg-slate-900/60 backdrop-blur-xl text-white flex items-center justify-between px-10 z-[90] relative shadow-2xl border-b border-white/5">
          <div className="flex items-center gap-6">
            <div className="relative">
              <h2 className="text-2xl font-black tracking-tighter uppercase italic">Institutional Dashboard</h2>
              <div className="absolute -bottom-1 left-0 w-full h-0.5 bg-teal-500 rounded-full shadow-[0_0_10px_#14b8a6]" />
            </div>
            <div className="h-6 w-px bg-white/10 mx-2" />
            <span className="text-slate-400 text-[10px] font-black uppercase tracking-[0.3em] bg-white/5 px-4 py-1.5 rounded-full border border-white/5">{activeTab} node</span>
          </div>
        ...
        {/* Global Alert Banner (Optional) */}
        <div className="bg-orange-500/10 border-y border-orange-500/20 px-10 py-2 hidden">
          <p className="text-[10px] font-black text-orange-400 uppercase tracking-widest text-center">System Advisory: Final Semester Assessments Authorized for Data Ingestion</p>
        </div>
