    ...
    <DashboardLayout>
      <div className="min-h-screen bg-[#1e1b4b] p-4 lg:p-10 space-y-10">
        ...
        <div className="bg-slate-900/40 backdrop-blur-3xl rounded-[32px] p-4 border border-white/5 shadow-3xl flex gap-4 overflow-x-auto items-center">
          ...
          <button onClick={() => setShowSelfCheckModal(true)} className="px-8 py-3 bg-white/5 border border-white/5 text-slate-500 hover:text-white font-black text-[10px] uppercase tracking-[0.2em] rounded-2xl transition-all">Self-Assessment</button>
          ...
          <div className="space-y-10">
              ...
              {notifications.length > 0 ? (
                <div className="space-y-6 relative z-10">
                  {notifications.slice(0, 3).map((note, i) => (
                    <div key={i} className="border-l-4 border-teal-600 pl-6 py-2 bg-slate-950/40 rounded-r-3xl group hover:bg-slate-950 transition-all">
                      <h4 className="font-black text-xs text-white uppercase tracking-tight group-hover:text-teal-400 transition-colors">{note.title}</h4>
                      <p className="text-[11px] text-slate-500 mt-2 font-bold line-clamp-2 italic opacity-70 group-hover:opacity-100 transition-all">"{note.message}"</p>
                    </div>
                  ))}
                </div>
              ) : (
            ...
            <div className="bg-slate-900/40 backdrop-blur-3xl rounded-[40px] p-10 border border-white/5 shadow-3xl relative overflow-hidden">
              <div className="flex justify-between items-center mb-8">
                ...
                <button onClick={() => setShowAddEventModal(true)} className="h-8 px-4 bg-teal-600/10 hover:bg-teal-600 text-teal-400 hover:text-white text-[9px] font-black uppercase tracking-widest rounded-xl transition-all border border-teal-500/20">+</button>
              </div>
